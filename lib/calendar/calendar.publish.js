(function($) {
	...
	/*---- Events -----------------------------------------------------------*/
		...
		// Visualise
		manager.delegate(settings.item, 'visualise.calendar setdate.datetime', function(event, range, focus) {
			var item = $(this);
		...
		manager.delegate('td', 'click.calendar', function(event) {
			...
			// Set date
			choose(calendar, timestamp, event.shiftKey);
		});
	...
	/*---- Functions --------------------------------------------------------*/
		...
		var visualise = function(item, date, range) {
			var calendar = item.find(settings.calendar),
				...
				now = new Date(),
				start = reduce(range.start),
				end = reduce(range.end),
		...
		var choose = function(calendar, selected, key) {
			...
			// Trigger set
			item.trigger('setdate', [range, selected]);
		};
