(function($) {
	...
	/*---- Events -----------------------------------------------------------*/
		...
		// Switching
		manager.delegate('span.nav a', 'click.calendar', function(event) {
		...
		manager.delegate('select', 'change.calendar', function(event) {
			var calendar = $(this).parents(settings.calendar),
				item = calendar.parents(settings.item),
				month = calendar.find('select.month').val(),
				year = calendar.find('select.year').val();
	...
	/*---- Functions --------------------------------------------------------*/
		...
		var visualise = function(item, date, range) {
			var calendar = item.find(settings.calendar),
				...
				years = calendar.find('select.year'),
				current, today,
				day, month, year,
				first, distance;
			...
			// Set month
			calendar.find('select.month').val(current.month);
			
			// Set year
			years.find('option').remove();
			for(y = current.year - 10; y <= current.year + 10; y++) {
				$('<option>' + y + '</option>').prependTo(years);
			}
			years.val(current.year);
