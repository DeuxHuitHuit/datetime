(function($) {
	...
	/*---- Functions --------------------------------------------------------*/
		...
		// Select time
		var select = function(timeline, position) {
			...
			// Trigger set time event
			timeline.parents(settings.item).trigger('settime', [hours, minutes]);
		};
		...
		var setTimerPosition = function(timeline) {
			...
			if(timeline.is('.start')) {
				...
				time.css('right', Math.min(width - parseInt(time.width()), 0));
			}			
		};
