(function($) {
	...
	/*---- Functions --------------------------------------------------------*/
		...
		var choose = function(timeline, position, key) {
			var end = timeline.parent().find('timeline.end'),
			...
			if(key) {
				...
				// New end, multiple days
				if(timeline.is('.end')) {
					timeline.parents(settings.item).trigger('settime', [current.from, selected, 'multiple', 'end']);
				}
				
				// New start, multiple days
				else if(end.is(':visible') && (current.from.hours * 100 + current.from.minutes) > (selected.hours * 100 + selected.minutes)) {
					timeline.parents(settings.item).trigger('settime', [selected, current.to, 'multiple', 'start']);
				}
				
				// New end, single day
				else if(!end.is(':visible') && (current.from.hours * 100 + current.from.minutes) < (selected.hours * 100 + selected.minutes)) {
					timeline.parents(settings.item).trigger('settime', [current.from, selected, 'single', 'end']);
				}
				
				// New start, single day
				else {
